...
const statusallDiceRegex = /\d+d(?:100|20|12|10|8|6|4)(?:kh\d+|kl\d+|ro(<|<=|>|>=|=)\d+)*/g; // ([numbers]d[diceTypes]kh[numbers] or [numbers]d[diceTypes]kl[numbers]) or [numbers]d[diceTypes]
const statusvalidExpressionRegex = /^[dkhlro<=>\s\d+\-\(\)]*$/g; // any of these [d, kh, kl, spaces, numbers, +, -] // Should we support [*, /] ?
const statusvalidModifierSubstitutions = /(?<!\w)(str|dex|con|int|wis|cha|pb)(?!\w)/gi // case-insensitive shorthand for stat modifiers as long as there are no letters before or after the match. For example `int` and `STR` would match, but `mint` or `strong` would not match.
const statusdiceRollCommandRegex = /^\/(r|roll|save|hit|dmg|skill|heal)\s/; // matches only the slash command. EG: `/r 1d20` would only match `/r`
const statusmultiDiceRollCommandRegex = /\/(r|roll|save|hit|dmg|skill|heal) [^\/]*/g; // globally matches the full command. EG: `note: /r 1d20 /r2d4` would find ['/r 1d20', '/r2d4']
const statusallowedExpressionCharactersRegex = /^(d\d|\d+d\d+|kh\d+|kl\d+|ro(<|<=|>|>=|=)\d+|\+|-|\d+|\s+|STR|str|DEX|dex|CON|con|INT|int|WIS|wis|CHA|cha|PB|pb)*/; // this is explicitly different from validExpressionRegex. This matches an expression at the beginning of a string while validExpressionRegex requires the entire string to match. It is also explicitly declaring the modifiers as case-sensitive because we can't search the entire thing as case-insensitive because the `d` in 1d20 needs to be lowercase.

class statusDiceRoll {
    ...
    constructor(expression, action = undefined, rollType = undefined, name = undefined, avatarUrl = undefined, entityType = undefined, entityId = undefined, sendToOverride = undefined) {
        ...
        if (!parsedExpression.match(statusvalidExpressionRegex)) {
        ...
        let separateDiceExpressions = parsedExpression.match(statusallDiceRegex)
    ...
    static fromSlashCommand(slashCommandText, name = undefined, avatarUrl = undefined, entityType = undefined, entityId = undefined, sendToOverride = undefined) {
        ...
        let slashCommand = modifiedSlashCommand.match(statusdiceRollCommandRegex)?.[0];
        let expression = modifiedSlashCommand.replace(statusdiceRollCommandRegex, "").match(statusallowedExpressionCharactersRegex)?.[0];
        let action = modifiedSlashCommand.replace(statusdiceRollCommandRegex, "").replace(statusallowedExpressionCharactersRegex, "");
        console.debug("statusDiceRoll.fromSlashCommand text: ", slashCommandText, ", slashCommand:", slashCommand, ", expression: ", expression, ", action: ", action);
        ...
        return new statusDiceRoll(expression, action, rollType, name, avatarUrl, entityType, entityId, sendToOverride);
    }
}

class statusDiceRoller {
    ...
    roll(diceRoll) {
        try {
            ...
            this.#pendingDiceRoll = new statusDiceRoll(diceRoll.expression, diceRoll.action, diceRoll.rollType, diceRoll.name, diceRoll.avatarUrl, diceRoll.entityType, diceRoll.entityId);

            this.clickDiceButtons(this.#pendingDiceRoll);
            console.groupEnd();
            return true;
        } catch (error) {
    ...
    #swapRollData(ddbMessage) {
        ...
        try {
            ...
            alteredMessage.data.rolls.forEach(r => {
                ...
                let rolledExpressions = r.diceNotationStr.match(statusallDiceRegex);
...
function replaceModifiersInSlashCommand(slashCommandText, entityType, entityId) {
    ...
    const expression = slashCommandText.replace(statusdiceRollCommandRegex, "").match(statusallowedExpressionCharactersRegex)?.[0];
    ...
    const modifiersToReplace = expression.matchAll(statusvalidModifierSubstitutions);
